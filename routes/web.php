...
use App\Http\Controllers\ShipStationOrder;
...
use App\Http\Controllers\AmazonSPApiController;
use App\Http\Controllers\AmazonProductController;
use App\Http\Controllers\ShopifyComplianceController;

Route::post('/webhook/shopify/customers/data_request', [ShopifyComplianceController::class, 'handleDataRequest']);
Route::post('/webhook/shopify/customers/redact', [ShopifyComplianceController::class, 'handleCustomerRedact']);
Route::post('/webhook/shopify/shop/redact', [ShopifyComplianceController::class, 'handleShopRedact']);

/////////******************************************************************************************* shopify */
use App\Http\Controllers\ShopifyController;

Route::get('/shopify/install', function () {
    return view('shopify.form'); // Blade view for prompting user input
})->name('shopify.form');
Route::get('/shopify/auth/', [ShopifyController::class, 'auth'])->name('shopify.auth');
Route::get('/shopify/callback', [ShopifyController::class, 'callback'])->name('shopify.callback');
//Route::get('/shopify/dashboard/{shop}', [ShopifyController::class, 'dashboard'])->name('shopify.dashboard');
/************************************************************************************************* */

/////////******************************************************************************************* woocommerce */
use App\Http\Controllers\WoocommerceController;

Route::get('/woo/authorize', [WoocommerceController::class, 'auth'])->name('woo.authorize');
Route::any('/woo/connect/callback/', [WoocommerceController::class, 'callback'])->name('woocommerce.callback');
Route::any('/woo/connect/response/', [WoocommerceController::class, 'response'])->name('woocommerce.response/');
...
Route::get('/amazon/listings/{sellerId}', [AmazonProductController::class, 'getAllProductListings']);

Route::get('/amazon-products', [AmazonSPApiController::class, 'getProducts']);
Route::get('/shipstation/create_order/{order_id}', [ShipStationOrder::class, 'createOrder'])->name('sale.ship');
...
$redirect_uri = 'https://dev.easternfulfillment.com/amazon1/authorize';
$state = 'US';// Pass this state into the redirect handler to verify the redirect request.
...
Route::get('/amazon1/authorize', function (Request $request) {
  ...
  //  dd($request->all());
    $provider = new GenericProvider([
        ...
        'redirectUri'             => 'https://dev.easternfulfillment.com/dashboard/',
 ...
 //   try {
     ...
     $accessToken = $provider->getAccessToken('refresh_token', [
        ...
        $marketplaceId = $request->selling_partner_id; // The Marketplace ID you're operating in
*/
$params = [ // You can also pass $_GET instead of a new array
    'state' => $request->state,
    'selling_partner_id' => $request->selling_partner_id,
    'spapi_oauth_code' => $request->spapi_oauth_code,
 //  'redirect_url'=>'https://dev.easternfulfillment.com/dashboard/'
     
];

  $redirect_uri='https://dev.easternfulfillment.com/dashboard/';
  $config = new AmznSPAConfig(
    lwa_refresh_token: env('AMAZON_SP_API_REFRESH_TOKEN'),
     marketplace_id: 'ATVPDKIKX0DER',
             lwa_client_id : env('AMAZON_SP_API_CLIENT_ID'), // Client ID from Amazon
        lwa_client_secret: env('AMAZON_SP_API_CLIENT_SECRET'), // Client Secret from Amazon
     application_id:'amzn1.sp.solution.0087b739-d0b7-42cb-9503-96c00d357ed1',
      redirect_url:'https://dev.easternfulfillment.com/dashboard/',
  ...
  $spa = new AmznSPA($config);
     $marketplace_id= 'ATVPDKIKX0DER';
$tokens = $spa->lwa->getTokensFromRedirect('US', $params); 
...
Route::group(['middleware' => [ 'active','auth','common']], function() {
Route::resource('integrations', IntegrationsController::class);
    ...
    Route::controller(HomeController::class)->group(function () {
        Route::get('/app_index', 'index')->name('app_index');
...
/************************************* */
    Route::controller(IntegrationController::class)->group(function () {
        ...
        Route::post('integration/deletebyselection', 'deleteBySelection');
        Route::get('integration/lims_integration_search', 'limsWarehouseSearch')->name('integration.search');
        Route::get('integration/all', 'integrationAll')->name('integration.all');
    });
...
Route::controller(PackingController::class)->group(function () {
    ...
    Route::post('packing/deletebyselection', 'deleteBySelection');
    Route::get('packing/lims_packing_search', 'limsWarehouseSearch')->name('integration.search');
    Route::get('packing/all', 'packingAll')->name('packing.all');
});
...
/********************************************************************************** */
    Route::controller(IntegrationsController::class)->group(function () {
        Route::prefix('integrations1')->group(function () {
            
        Route::get('/getsync/{id}', 'getsync');
             
          Route::post('integration-data', 'integrationData')->name('integration.data');
          //  Route::get('product_integration/{id}', 'productIntegrationData');
          //  Route::get('lims_product_search', 'limsProductSearch')->name('product_purchase.search');
         //   Route::post('add_integration', 'addIntegration')->name('integration.add-integration');
            Route::get('getintegration/{id}', 'getIntegration')->name('purchase.get-integration');
         //   Route::post('updateintegration', 'updateIntegration')->name('integration.update-integration');
        //    Route::post('deleteintegration', 'deleteIntegration')->name('integration.delete-integration');
           // Route::get('purchase_by_csv', 'integrationByCsv');
          //  Route::get('duplicate/{id}', 'duplicate')->name('integration.duplicate');
         //   Route::post('deletebyselection', 'deleteBySelection');
        });
    });
  //  Route::resource('integrations', IntegrationsController::class);
...
Route::get('/register_eastern', function () {
    return view('register_eastern');
})->name('register_eastern');

Route::post('register_eastern', [RegisteredUserController::class,'store'])->name('register_eastern');
...
Route::get('/payment', [StripePaymentController::class, 'redirectToStripe'])->name('payment');
Route::get('/payment/complete', [StripePaymentController::class, 'handleCheckoutCompleted'])->name('payment.complete');
/*
