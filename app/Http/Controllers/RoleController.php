...
/************************** */
if($request->has('integrations-index')){
    $permission = Permission::firstOrCreate(['name' => 'integrations-index']);
    if(!$role->hasPermissionTo('integrations-index')){
        $role->givePermissionTo($permission);
    }
}
else
    $role->revokePermissionTo('integrations-index');

    
    if($request->has('integrations-add')){
        $permission = Permission::firstOrCreate(['name' => 'integrations-add']);
        if(!$role->hasPermissionTo('integrations-add')){
            $role->givePermissionTo($permission);
        }
    }
    else
        $role->revokePermissionTo('integrations-add');

    if($request->has('integrations-edit')){
        $permission = Permission::firstOrCreate(['name' => 'integrations-edit']);
        if(!$role->hasPermissionTo('integrations-edit')){
            $role->givePermissionTo($permission);
        }
    }
    else
        $role->revokePermissionTo('integrations-edit');

    if($request->has('integrations-delete')){
        $permission = Permission::firstOrCreate(['name' => 'integrations-delete']);
        if(!$role->hasPermissionTo('integrations-delete')){
            $role->givePermissionTo($permission);
        }
    }
    else
        $role->revokePermissionTo('integrations-delete');
/**************************** */
        ...
        if($request->has('sale-payment-index')){
            ...
            if($request->has('sale-ship')){
                $permission = Permission::firstOrCreate(['name' => 'sale-ship']);
                if(!$role->hasPermissionTo('sale-ship')){
                    $role->givePermissionTo($permission);
                }
            }
            else
                $role->revokePermissionTo('sale-ship');
        ...
        if($request->has('warehouse')){
            ...
            $role->revokePermissionTo('integration');
        /***************** */
        if($request->has('integration')){
                $permission = Permission::firstOrCreate(['name' => 'integration']);
                if(!$role->hasPermissionTo('integration')){
                    $role->givePermissionTo($permission);
                }
        }
        else
        ...
        if($request->has('packing')){
            $permission = Permission::firstOrCreate(['name' => 'packing']);
            if(!$role->hasPermissionTo('packing')){
                $role->givePermissionTo($permission);
            }
        }
        else
            $role->revokePermissionTo('packing');
        /**************************************** */
