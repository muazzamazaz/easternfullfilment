...
class SaleController extends Controller
{
    ...
    public function saleData(Request $request)
    {
        ...
        if(!empty($sales))
        {
            foreach ($sales as $key=>$sale)
            {
                ...
                $nestedData['reference_no'] = $sale->reference_no; Log::info($sale->reference_no);
                ...
                $nestedData['options'] = '<div class="btn-group">
                            ...
                            if(!empty($sale->packing_id) && empty($sale->ship_order))
                                $nestedData['options'] .=  '<li><a href="'.route('sale.ship', $sale->id).'" class="btn btn-link"><i class="fa fa-ship"></i> '.trans('file.Ship').'</a></li>';
                            elseif(!empty($sale->packing_id) && !empty($sale->ship_order))
                                $nestedData['options'] .=  '<li><a class="btn btn-link"><i class="fa fa-ship"></i> '.trans('Already Shipped').'</a></li>';
    ...
    public function edit($id)
    {
        ...
        if($role->hasPermissionTo('sales-edit')){
            $lims_customer_list = Customer::where('is_active', true)->get();
            $lims_warehouse_list = Warehouse::where('is_active', true)->get();
            $lims_packing_list = Packing::where('is_active', true)->get();
            $lims_biller_list = Biller::where('is_active', true)->get();
            $lims_tax_list = Tax::where('is_active', true)->get();
            $lims_sale_data = Sale::find($id);
            $lims_product_sale_data = Product_Sale::where('sale_id', $id)->get();
            ...
            $custom_fields = CustomField::where('belongs_to', 'sale')->get();
            return view('backend.sale.edit',compact('lims_customer_list','lims_packing_list', 'lims_warehouse_list', 'lims_biller_list', 'lims_tax_list', 'lims_sale_data','lims_product_sale_data', 'currency_exchange_rate', 'custom_fields'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
